/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PromptCategoryWhereInput } from "./PromptCategoryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PromptCategoryOrderByInput } from "./PromptCategoryOrderByInput";

@ArgsType()
class PromptCategoryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PromptCategoryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PromptCategoryWhereInput, { nullable: true })
  @Type(() => PromptCategoryWhereInput)
  where?: PromptCategoryWhereInput;

  @ApiProperty({
    required: false,
    type: [PromptCategoryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PromptCategoryOrderByInput], { nullable: true })
  @Type(() => PromptCategoryOrderByInput)
  orderBy?: Array<PromptCategoryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PromptCategoryFindManyArgs as PromptCategoryFindManyArgs };
